...
class ServerWorker:
	...
	def sendRtp(self):
		...
		while True:
			...
			if data: 
				...
				if flag:
					self.replyRtsp(self.OK_200_SEND_CPL,self.seq)
					flag = False
	...
	def replyRtsp(self, code, seq):
		...
		if code == self.OK_200:
			...
			# print(hostname)
			info = '\nType Stream: video\tName: '+self.filename + '\tServer: '+ hostname + '\tProtocol: RTP/AVP Video\tFormat: MJPEG\tLenght video: ' + str(self.clientInfo['videoStream'].lenVideo()) +' bytes'
			reply = 'RTSP/1.0 200 OK\nCSeq: ' + seq + '\nSession: ' + str(self.clientInfo['session']) + info
		elif code == self.OK_200_SEND_CPL:
			reply = 'RTSP/1.0 200 OK\nCSeq: ' + seq + '\nSession: ' + str(self.clientInfo['session']) +'\nTotalPacket:\n'+ str(self.numRtpSend)
		if reply:
			connSocket = self.clientInfo['rtspSocket'][0]
